// Systematic test to isolate SEGFAULT cause
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let test_cases = vec![
        ...
        ("With basic style", r#"<!DOCTYPE html><html><head><style>div { display: block; }</style></head><body><div>Test</div></body></html>"#),
        ...
        ("With box model", r#"<!DOCTYPE html><html><head><style>div { margin: 10px; padding: 10px; border: 1px solid black; }</style></head><body><div>Test</div></body></html>"#),
        ...
        ("Position absolute", r#"<!DOCTYPE html><html><head><style>div { position: absolute; top: 10px; left: 10px; }</style></head><body><div>Test</div></body></html>"#),
    ...
    for (name, html) in test_cases {
        ...
        match page.set_content(html).await {
            Ok(_) => println!("✅ set_content() succeeded"),
            Err(e) => {
                println!("❌ set_content() FAILED: {}", e);
        ...
        match eval_result {
            Ok(Ok(_)) => println!("✅ evaluate() succeeded - NO CRASH"),
            Ok(Err(e)) => println!("❌ evaluate() failed: {}", e),
            Err(_) => println!("⏱️ evaluate() TIMEOUT - likely crashed"),
        }
