...
fn rect_to_scissor(rect: (f32, f32, f32, f32), framebuffer_size: (u32, u32)) -> Scissor {
    ...
    let final_width = if max_w <= 0 {
        0
    } else {
        u32::try_from(scissor_width.clamp(0i32, max_w)).unwrap_or(0)
    };
    let final_height = if max_h <= 0 {
        0
    } else {
        u32::try_from(scissor_height.clamp(0i32, max_h)).unwrap_or(0)
    };
    (
