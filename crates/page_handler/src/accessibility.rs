...
pub fn ax_tree_snapshot_from(
    ...
    fn name_for(kind: &LayoutNodeKind, key: NodeKey, attrs_map: &HashMap<NodeKey, HashMap<String, String>>) -> String {
    ...
    fn serialize(node: NodeKey,
                 kind_by_key: &HashMap<NodeKey, LayoutNodeKind>,
                 children_by_key: &HashMap<NodeKey, Vec<NodeKey>>,
                 attrs_map: &HashMap<NodeKey, HashMap<String, String>>) -> String {
        let mut out = String::new();
        let kind = kind_by_key.get(&node).cloned().unwrap_or(LayoutNodeKind::Document);
        ...
        out.push_str("{\"role\":\""); out.push_str(role); out.push('"');
        if !name.is_empty() { out.push_str(",\"name\":\""); out.push_str(&name); out.push('"'); }
