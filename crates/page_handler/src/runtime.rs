...
pub trait JsRuntime {
    ...
    fn drive_after_dom_update<'a>(&'a mut self, page: &'a mut HtmlPage) -> core::pin::Pin<Box<dyn core::future::Future<Output = Result<(), Error>> + 'a>>;
}
...
impl JsRuntime for DefaultJsRuntime {
    ...
    fn tick_timers_once(&mut self, page: &mut HtmlPage) { page.tick_js_timers_once(); }
    fn drive_after_dom_update<'a>(&'a mut self, page: &'a mut HtmlPage) -> core::pin::Pin<Box<dyn core::future::Future<Output = Result<(), Error>> + 'a>> {
        Box::pin(async move {
