...
pub fn selection_rects(
    ...
    let selection = LayoutRect { x: sel_x, y: sel_y, width: sel_w, height: sel_h };
    ...
    for (key, kind, _children) in snapshot.iter() {
        if let LayoutNodeKind::InlineText { text } = kind {
            ...
            if let Some(r) = rects.get(key) {
                ...
                let iw = (ix1 - ix).max(0);
                let ih = (iy1 - iy).max(0);
                if iw > 0 && ih > 0 { out.push(LayoutRect { x: ix, y: iy, width: iw, height: ih }); }
            }
        }
    }
    out
...
pub fn caret_at(
    ...
    if let Some(hit_key) = hit
        ...
        && let Some((_k, LayoutNodeKind::InlineText { .. }, _)) = snapshot.iter().find(|(k, _, _)| *k == hit_key)
    {
        let cx = x.clamp(r.x, r.x + r.width);
        return Some(LayoutRect { x: cx, y: r.y, width: 1, height: r.height });
    }
