use wgpu_renderer::{DisplayList, DisplayItem};
use js::NodeKey;
...
pub trait DisplayBuilder: Send + Sync {
    ...
    fn build_rect_list(&self, rects: &HashMap<NodeKey, layouter::LayoutRect>, snapshot: &[(NodeKey, layouter::LayoutNodeKind, Vec<NodeKey>)]) -> Vec<wgpu_renderer::DrawRect>;
    fn build_text_list(&self, rects: &HashMap<NodeKey, layouter::LayoutRect>, snapshot: &[(NodeKey, layouter::LayoutNodeKind, Vec<NodeKey>)], computed_map: &HashMap<NodeKey, style_engine::ComputedStyle>) -> Vec<wgpu_renderer::DrawText>;
}

pub struct DefaultDisplayBuilder;

impl DisplayBuilder for DefaultDisplayBuilder {
    fn build_retained(&self, inputs: RetainedInputs) -> DisplayList { build_retained(inputs) }
    fn build_rect_list(&self, rects: &HashMap<NodeKey, layouter::LayoutRect>, snapshot: &[(NodeKey, layouter::LayoutNodeKind, Vec<NodeKey>)]) -> Vec<wgpu_renderer::DrawRect> {
...
pub fn build_text_list(
    ...
    for (parent, _kind, children) in snapshot.iter() {
        for &c in children { parent_of.insert(c, *parent); }
    }
    ...
    let nearest_style = |start: NodeKey| -> Option<&style_engine::ComputedStyle> {
        ...
        while let Some(n) = cur {
            if let Some(cs) = computed_map.get(&n) { return Some(cs); }
    ...
    for (key, kind, _children) in snapshot.iter() {
        if let layouter::LayoutNodeKind::InlineText { text } = kind {
            ...
            if let Some(rect) = rects.get(key) {
                ...
                list.push(wgpu_renderer::DrawText { x: rect.x as f32, y: rect.y as f32, text: display_text, color: color_rgb, font_size });
            }
        }
    }
    list
...
pub fn build_retained(inputs: RetainedInputs) -> DisplayList {
    ...
    for (parent, children) in children_map.iter() { for &c in children { parent_map.insert(c, *parent); } }

    // Helper: nearest ancestor with a computed style (for inline text, etc.)
    fn nearest_style<'a>(start: NodeKey, parent_map: &HashMap<NodeKey, NodeKey>, computed_map: &'a HashMap<NodeKey, style_engine::ComputedStyle>, computed_fallback: &'a HashMap<NodeKey, style_engine::ComputedStyle>, computed_robust: &'a Option<HashMap<NodeKey, style_engine::ComputedStyle>>) -> Option<&'a style_engine::ComputedStyle> {
        let mut cur = Some(start);
        while let Some(n) = cur {
            if let Some(cs) = computed_robust.as_ref().and_then(|m| m.get(&n))
        ...
        match kind {
            layouter::LayoutNodeKind::Document => {
                if let Some(children) = children_map.get(&node) {
                    for &child in children { recurse(list, child, kind_map, children_map, rects, computed_map, computed_fallback, computed_robust, parent_map); }
                }
            }
            layouter::LayoutNodeKind::Block { .. } => {
                if let Some(rect) = rects.get(&node) {
                    ...
                    let cs_opt = computed_robust.as_ref().and_then(|m| m.get(&node))
                        .or_else(|| computed_fallback.get(&node))
                        .or_else(|| computed_map.get(&node));
                    if let Some(cs) = cs_opt {
                        let bg = cs.background_color;
                        rgba = [bg.red as f32 / 255.0, bg.green as f32 / 255.0, bg.blue as f32 / 255.0, bg.alpha as f32 / 255.0];
                    }
                    if rgba[3] > 0.0 {
                        list.push(DisplayItem::Rect { x: rect.x as f32, y: rect.y as f32, width: rect.width as f32, height: rect.height as f32, color: rgba });
                    }
                    // overflow clip (style-driven via nearest element style)
                    let mut opened_clip = false;
                    let style_for_node = nearest_style(node, parent_map, computed_map, computed_fallback, computed_robust);
                    let overflow_hidden = style_for_node.map(|cs| matches!(cs.overflow, style_engine::Overflow::Hidden)).unwrap_or(false);
                    if overflow_hidden {
            ...
            layouter::LayoutNodeKind::InlineText { text } => {
                ...
                if let Some(rect) = rects.get(&node) {
                    let (font_size, color_rgb) = if let Some(cs) = nearest_style(node, parent_map, computed_map, computed_fallback, computed_robust) {
                        let c = cs.color; (cs.font_size, [c.red as f32 / 255.0, c.green as f32 / 255.0, c.blue as f32 / 255.0])
                    } else { (16.0, [0.0, 0.0, 0.0]) };
    ...
    recurse(&mut list, NodeKey::ROOT, &kind_map, &children_map, &rects, &computed_map, &computed_fallback, &computed_robust, &parent_map);

    if let Some((x0, y0, x1, y1)) = selection_overlay {
        let sel_x = x0.min(x1); let sel_y = y0.min(y1);
        let sel_w = (x0.max(x1) - sel_x).max(0); let sel_h = (y0.max(y1) - sel_y).max(0);
        ...
        for (_k, rect) in rects.iter() {
            ...
            let ih = (iy1 - iy).max(0);
            if iw > 0 && ih > 0 {
                list.push(DisplayItem::Rect { x: ix as f32, y: iy as f32, width: iw as f32, height: ih as f32, color: [0.2, 0.5, 1.0, 0.35] });
            }
        }
    }
    ...
    if hud_enabled {
        let hud = format!("restyled:{} spill:{}", last_style_restyled_nodes, spillover_deferred);
