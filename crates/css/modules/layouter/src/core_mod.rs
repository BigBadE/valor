//! Core orchestration for the layouter: entry points and root aggregation.
...
use super::{ContainerMetrics, LayoutNodeKind, Layouter, RootHeightsCtx};
...
pub fn compute_container_metrics_impl(
    ...
    let margin_left = 0i32; // Root margin-left simplified to 0 in MVP
    let margin_top = 0i32; // Root margin-top simplified to 0 in MVP
...
pub fn layout_root_impl(layouter: &mut Layouter) -> usize {
    ...
    let icb_width: i32 = super::INITIAL_CONTAINING_BLOCK_WIDTH;
    ...
    let (content_height, root_height_border_box) = super::heights::compute_root_heights_impl(
        layouter,
        RootHeightsCtx {
            root,
            metrics,
...
pub fn push_dirty_rect_if_changed_impl(
    ...
    if reflowed_count > 0 {
        layouter.dirty_rects.push(super::LayoutRect {
            x: 0,
            y: 0,
            width,
