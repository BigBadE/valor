...
use style_engine::{Clear, ComputedStyle, Position};
...
impl Layouter {
    ...
    fn layout_block_children(
        ...
        if !block_children.is_empty() {
            ...
            let (placed, y_end, last_mb) =
                self.place_block_children_loop(loop_ctx, &mut first_collapsed_top_positive);
            reflowed_count = placed;
            y_cursor = y_end;
            last_positive_bottom_margin = last_mb.max(0i32);
        }
    ...
    #[inline]
    fn collect_block_children(&self, root: NodeKey) -> Vec<NodeKey> {
        let child_list =
    ...
    fn place_block_children_loop(
        ...
        (reflowed_count, y_cursor, previous_bottom_margin)
    }
    ...
    fn compute_heights_and_margins(&mut self, hctx: HeightsCtx<'_>) -> HeightsAndMargins {
        ...
        if (hctx.sides.padding_bottom > 0i32 || hctx.sides.border_bottom > 0i32)
            && last_pos_mb > 0i32
        {
    ...
    fn log_vert(entry: VertLog) {
        diagnostics::log_vert_impl(entry);
    }
    ...
    fn compute_collapsed_vertical_margin(
        ...
        // If the child requests clearance, do not collapse its top margin with the previous sibling.
        if !ctx.is_first_placed
            && matches!(child_style.clear, Clear::Left | Clear::Right | Clear::Both)
        {
            debug!(
                "[VERT-COLLAPSE sibling] clear present -> no collapse with prev, collapsed_top={child_margin_top}"
            );
