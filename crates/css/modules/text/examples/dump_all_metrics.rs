...
fn main() {
    ...
    if let Some(first_match) = font_matches.first() {
        ...
        println!("hhea metrics (from metrics()):");
        ...
        if let Some((win_ascent, win_descent)) = font.os2_metrics() {
            ...
            println!("  Total (win): {}", win_ascent + win_descent);
            println!();
        ...
        println!("=== Comparison ===");
        println!("Chrome Canvas2D reported for 14px:");
        println!("  fontBoundingBoxAscent: 15px");
        println!("  fontBoundingBoxDescent: 4px");
        println!("  Total: 19px");
    } else {
        println!("Liberation Sans not found!");
    }
}
