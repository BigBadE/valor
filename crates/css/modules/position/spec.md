# CSS Positioned Layout — Spec Coverage Map (CSS Position 3)

Primary spec: https://www.w3.org/TR/css-position-3/

## Scope and maturity

- Status: [MVP]
- Non-production items: [TODO] Full offset resolution, formatting integration with BFC/IFC; [Approximation] initial absolute/fixed containing block and offsets.
- Out of scope (for now): sticky and anchor positioning; fragmentation.

## One-to-one spec mapping

This information is integrated into the verbatim spec below via per-section blocks. Use this section only for cross-spec mappings that don’t belong to the primary TR (e.g., some z-index/stacking context rules in CSS 2.2/CSS Display).

## Algorithms and data flow

- Entry points: TBD under `position/` chapter files mirroring the spec.
- Helpers: TBD for offset resolution and containing block discovery.
- Invariants: Positioned elements are out-of-flow; do not affect in-flow siblings' placement.

## Parsing/inputs (if applicable)

- Inputs: `ComputedStyle.position`, offsets `top/left/right/bottom` (px or %), `z-index` per `css_orchestrator`.

## Integration

- Upstream: `css_orchestrator::style_model::ComputedStyle` values (position, offsets, z-index).
- Downstream: `css_core` layouter for rect assignment; `page_handler::display` ordering buckets for z-index positioning.

## Edge cases

- Percentage offsets resolution depends on containing block dimensions; TBD for replaced vs non-replaced.

## Testing and fixtures

- Fixtures: `crates/css/modules/position/tests/fixtures/layout/positioning/`
- Ignored fixtures: `.fail` files are skipped by discovery.
...
- [ ] Containing block discovery for absolute/fixed (§ formatting contexts and root viewport)
- [ ] Offset resolution for percentages (§ offsets)
- [ ] Size resolution when width/height are auto (§ formatting of abspos)
- [ ] Stacking context integration with z-index and opacity

---

# Verbatim Spec (CSS Positioned Layout Level 3)
...
- Use PowerShell (Windows) or Bash (Unix) vendor scripts:
  - PowerShell: `./scripts/vendor_display_spec.ps1 -SpecUrl "https://www.w3.org/TR/css-position-3/" -ModuleSpecPath "crates/css/modules/position/spec.md" -Year "2025"`
  - Bash: `./scripts/vendor_display_spec.sh "https://www.w3.org/TR/css-position-3/" "crates/css/modules/position/spec.md" "2025"`
- After generation, insert per-section status/mapping blocks after each H2/H3 heading and before the `<details class="valor-spec">` for that section.
- Do not edit inside the auto-generated verbatim region. Keep vendor markers intact so the script can re-run idempotently.
...
<div data-valor-status="position-property">
  <p><strong>Status:</strong> [Production] for <em>absolute</em> and <em>fixed</em>; [MVP] for others.</p>
  <p><strong>Code:</strong> <code>css_core::orchestrator::place_child::handle_out_of_flow_positioned</code>, <code>css_core::orchestrator::place_child::containing_block</code>, <code>css_core::orchestrator::place_child::resolve_all_offsets</code>, <code>css_core::orchestrator::place_child::compute_used_sizes</code>, <code>css_core::orchestrator::place_child::choose_position</code></p>
  <p><strong>Fixtures:</strong> <em>See position module fixtures under</em> <code>crates/css/modules/position/tests/fixtures/layout/positioning/</code></p>
  <p><strong>Notes:</strong> <em>absolute</em> and <em>fixed</em> are out-of-flow; we implement offset resolution and auto-size rules. Sticky/anchor are deferred.</p>
  <p><strong>Spec:</strong> §2</p>
</div>
...
<div data-valor-status="def-cb">
  <p><strong>Status:</strong> [Production] for <em>absolute</em> nearest positioned ancestor padding-box and <em>fixed</em> viewport; [MVP] for inline-fragment CB corner cases.</p>
  <p><strong>Code:</strong> <code>css_core::orchestrator::place_child::containing_block</code></p>
  <p><strong>Fixtures:</strong> <em>Pending additions</em></p>
  <p><strong>Notes:</strong> Inline-fragment multi-line CBs and grid-area adjustments are tracked for future work.</p>
  <p><strong>Spec:</strong> §2.1</p>
</div>
...
<div data-valor-status="stacking">
  <p><strong>Status:</strong> [MVP]</p>
  <p><strong>Code:</strong> <code>page_handler::display::z_key_for_child</code></p>
  <p><strong>Fixtures:</strong> <code>crates/css/modules/display/tests/fixtures/layout/stacking/zindex_simple.html</code></p>
  <p><strong>Notes:</strong> Stacking contexts are simplified; positioned buckets match CSS2 order. Full SC creation via properties (opacity/transform) is tracked.</p>
  <p><strong>Spec:</strong> §2.2</p>
</div>
...
<div data-valor-status="coords">
  <p><strong>Status:</strong> [Production] for physical insets (<code>top/right/bottom/left</code>) in absolute/fixed schemas.</p>
  <p><strong>Code:</strong> <code>css_orchestrator::style::apply_offsets</code>, <code>css_core::orchestrator::place_child::resolve_all_offsets</code></p>
  <p><strong>Fixtures:</strong> <em>Pending additions</em></p>
  <p><strong>Notes:</strong> Vertical percentage offsets resolve against CB height when determinable; otherwise 0 for abspos.</p>
  <p><strong>Spec:</strong> §3</p>
</div>
...
<div data-valor-status="insets">
  <p><strong>Status:</strong> [Production] for physical insets in absolute/fixed; [MVP] for logical insets.</p>
  <p><strong>Code:</strong> <code>css_orchestrator::style::apply_offsets</code></p>
  <p><strong>Fixtures:</strong> <em>Pending additions</em></p>
  <p><strong>Notes:</strong> Logical inset mapping deferred.</p>
  <p><strong>Spec:</strong> §3.1</p>
</div>
...
<div data-valor-status="inset-shorthands">
  <p><strong>Status:</strong> [MVP]</p>
  <p><strong>Code:</strong> <code>css_orchestrator::style::apply_offsets</code></p>
  <p><strong>Fixtures:</strong> <em>Pending additions</em></p>
  <p><strong>Notes:</strong> Shorthand parsing for logical insets is not fully covered.</p>
  <p><strong>Spec:</strong> §3.2</p>
</div>
...
<!--__VALOR_STATUS:relpos-insets__-->
...
<!--__VALOR_STATUS:stickypos-insets__-->
...
<!--__VALOR_STATUS:abspos-insets__-->
...
<!--__VALOR_STATUS:abspos-layout__-->
...
<!--__VALOR_STATUS:abspos-auto-size__-->
...
<!--__VALOR_STATUS:abspos-margins__-->
...
<!--__VALOR_STATUS:abspos-alignment__-->
...
<!--__VALOR_STATUS:abs-non-replaced-width__-->
...
<!--__VALOR_STATUS:abs-replaced-width__-->
...
<!--__VALOR_STATUS:abs-non-replaced-height__-->
...
<!--__VALOR_STATUS:abs-replaced-height__-->
...
<!--__VALOR_STATUS:comp-normal-flow__-->
...
<!--__VALOR_STATUS:comp-floating__-->
