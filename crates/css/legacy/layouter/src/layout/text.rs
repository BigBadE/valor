...
// Shaping and font system are always enabled
...
pub fn reorder_bidi_for_display(input: &str) -> String { input.to_string() }
...
pub fn measure_text_width(text: &str, font_size: f32, _fallback_char_width: i32) -> TextMetrics {
    if text.is_empty() {
        return TextMetrics { width: 0, line_height: (font_size * 1.2).round() as i32 };
    }

    // Cache lookup first
    let key: MeasureKey = (text.to_string(), font_size.round() as u32, 0);
    ...
    let out = TextMetrics { width, line_height };
    if let Ok(mut cache) = MEASURE_CACHE.lock() { cache.insert(key, out); }
    out
}
