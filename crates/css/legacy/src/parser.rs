...
fn normalize_shorthands(mut decls: Vec<Declaration>) -> Vec<Declaration> {
    ...
    for d in decls.drain(..) {
        ...
        match name.as_str() {
            ...
            "background" => {
                // Expand to background-color to satisfy tests and simplify style engine consumption.
                let v_owned = d.value.trim().to_string();
                out.push(emit_decl("background-color", &v_owned, d.important));
            }
            _ => out.push(d),
        }
    }
