...
impl StyleEngine {
    ...
    fn compute_for_node(&self, node: NodeKey) -> ComputedStyle {
        ...
        // Stacking
        let mut z_index_spec: Option<Option<i32>> = None; // None=unspecified; Some(None) => auto; Some(Some(n)) => value
        ...
        for (_imp_rank, _origin_rank, _spec, _ord, prop, val_raw) in items {
            ...
            match prop_lc.as_str() {
                ...
                "z-index" => {
                    ...
                    if v == "auto" { z_index_spec = Some(None); }
                    else if let Ok(n) = v.parse::<i32>() { z_index_spec = Some(Some(n)); }
                }
                // Font
                "font-size" => {
        ...
        let mut computed_display = display_spec.unwrap_or_else(|| default_display_for_tag(&info.tag));
        let computed_position = position_spec.unwrap_or(Position::Static);
        // CSS blockification: positioned elements generate block-level boxes regardless of inline outer display.
        // Inline → block; inline-flex → flex. Other inner display types follow their own rules, which we simplify here.
        if !matches!(computed_position, Position::Static) {
            computed_display = match computed_display {
        ...
        if let Some(zi) = z_index_spec { cs.z_index = zi; }
...
body { display: block; margin: 0; font-family: 'Noto Sans', 'Segoe UI', Arial, sans-serif; line-height: normal }
div, p, header, main, footer, section, article, nav, ul, ol, li, h1, h2, h3, h4, h5, h6 { display: block }
span, a, b, i, strong, em { display: inline }
code, pre, kbd, samp { font-family: 'Consolas', 'Courier New', monospace }
button { display: block; border-width: 2px; border-style: solid; padding: 1px 6px; line-height: normal; font-family: inherit }
...
impl StyleEngine {
    ...
    pub fn rebuild_from_layout_snapshot(
        ...
        children_by_key: &HashMap<js::NodeKey, Vec<js::NodeKey>>,
        attrs_by_key: &HashMap<js::NodeKey, HashMap<String, String>>,
    ) {
        use js::NodeKey;
