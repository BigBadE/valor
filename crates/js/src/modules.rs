...
impl SimpleFileModuleResolver {
    pub fn new() -> Self { Self { cache: HashMap::new(), visiting: HashMap::new() } }
    ...
    fn load_text(&mut self, url: &url::Url) -> anyhow::Result<String> {
        ...
        let txt = match url.scheme() {
            "file" => {
                ...
                let ext_ok = path.extension().and_then(|e| e.to_str()).map(|e| matches!(e.to_ascii_lowercase().as_str(), "js" | "mjs")).unwrap_or(false);
                if !ext_ok {
        ...
        for line in src.lines() {
            ...
            if s.starts_with("import ") {
                if let Some(idx) = s.rfind("\"").or_else(|| s.rfind("'")) {
                    let first = &s[..idx];
                    if let Some(startq) = first.rfind('"').or_else(|| first.rfind('\'')) {
...
impl ModuleResolver for SimpleFileModuleResolver {
    fn bundle_root(&mut self, root_spec: &str, base_url: &url::Url, inline_root_source: Option<&str>) -> anyhow::Result<String> {
        if let Some(src) = inline_root_source { return Ok(Self::strip_import_export(src)); }
        let root = self.resolve_spec(root_spec, base_url).ok_or_else(|| anyhow::anyhow!("bad specifier: {}", root_spec))?;
