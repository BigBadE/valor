...
pub fn update_valor_ui(
    mut query: Query<&mut ValorUiRoot>,
) {
    for mut ui_root in query.iter_mut() {
        if ui_root.dirty {
            // Mark as clean
            ui_root.dirty = false;
        }
...
pub fn handle_click_events(
    ...
    for event in click_events.read() {
        for mut ui_root in query.iter_mut() {
            ...
            info!("Click event at ({}, {}) on node {:?}", event.x, event.y, event.node);
            ui_root.dirty = true;
        }
    }
}
...
pub fn create_valor_ui(
    html: &str,
    callbacks: crate::events::EventCallbacks,
) -> Result<Vec<js::DOMUpdate>> {
    // Compile and inject HTML
    let mut vdom = crate::VirtualDom::new(
        {
