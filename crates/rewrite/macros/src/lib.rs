...
pub fn derive_query(input: TokenStream) -> TokenStream {
    ...
    let Data::Enum(data_enum) = &input.data else {
        panic!("Query can only be derived for enums");
    };
    ...
    for variant in &data_enum.variants {
        ...
        // Check for #[params(...)] attribute to specify marker traits
        let param_traits = variant.attrs.iter().find_map(|attr| {
            if attr.path().is_ident("params") {
                let Meta::List(meta_list) = &attr.meta else {
                    return None;
        ...
        match param_traits {
            None => {
                // No parameters - generate simple non-generic query struct
                let struct_name =
                ...
                generated_items.push(quote! {
                    ...
                    impl rewrite_core::Query for #struct_name {
                        ...
                        fn execute(db: &rewrite_core::Database, node: rewrite_core::NodeId, ctx: &mut rewrite_core::DependencyContext) -> Self::Value {
                            #query_fn(db, node, ctx)
                        }
                    }
                });
            }
            Some(ref params) if params.len() == 1 => {
                // One parameter - generate generic query struct
                let struct_name =
                    syn::Ident::new(&format!("{}Query", variant_name), variant_name.span());
                let trait_bound = &params[0];
                ...
                generated_items.push(quote! {
                        ...
                        Param1: #trait_bound + 'static,
                    {
                        ...
                        fn execute(db: &rewrite_core::Database, node: rewrite_core::NodeId, ctx: &mut rewrite_core::DependencyContext) -> Self::Value {
                            #query_fn(db, node, Param1::to_value(), ctx)
                        }
                    }
                });
            }
            Some(ref params) if params.len() == 2 => {
                // Two parameters - generate generic query struct
                let struct_name =
                    syn::Ident::new(&format!("{}Query", variant_name), variant_name.span());
                let trait_bound1 = &params[0];
                let trait_bound2 = &params[1];
                ...
                generated_items.push(quote! {
                        ...
                        Param2: #trait_bound2 + 'static,
                    {
                        ...
                        fn execute(db: &rewrite_core::Database, node: rewrite_core::NodeId, ctx: &mut rewrite_core::DependencyContext) -> Self::Value {
                            #query_fn(db, node, Param1::to_value(), Param2::to_value(), ctx)
                        }
                    }
                });
            }
