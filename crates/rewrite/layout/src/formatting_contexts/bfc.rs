...
use rewrite_css::{CssKeyword, CssValue, DisplayQuery, FloatQuery, OverflowQuery, PositionQuery};
...
pub fn establishes_bfc(scoped: &mut ScopedDb) -> bool {
    ...
    match display {
        ...
        CssValue::Keyword(CssKeyword::InlineBlock) => return true,

        // Table cells establish BFC
        CssValue::Keyword(CssKeyword::TableCell) => return true,

        // Flow-root explicitly creates BFC (CSS Display Level 3)
        CssValue::Keyword(CssKeyword::FlowRoot) => return true,
        ...
        | CssValue::Keyword(CssKeyword::InlineGrid) => return true,

        _ => {}
    }

    // 4. Check overflow (anything except visible establishes BFC)
    let overflow = scoped.query::<OverflowQuery>();
    if !matches!(overflow, CssValue::Keyword(CssKeyword::Visible)) {
        return true;
    }
...
pub(crate) use establishes_bfc as establishes_bfc_internal;
...
/// TODO: Implement when float support is added.
#[allow(dead_code)]
pub fn compute_clearance(_scoped: &mut ScopedDb) -> crate::Subpixels {
