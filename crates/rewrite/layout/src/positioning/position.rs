...
pub fn compute_positioned_offset<Axis>(
    scoped: &mut ScopedDb,
    ...
    Axis: crate::LayoutsMarker + 'static,
{
    ...
    match position {
        ...
        CssValue::Keyword(CssKeyword::Relative) => {
            compute_relative_offset::<Axis>(scoped, normal_flow_offset)
        }

        CssValue::Keyword(CssKeyword::Absolute) => {
            compute_absolute_offset::<Axis>(scoped, normal_flow_offset)
        }

        CssValue::Keyword(CssKeyword::Fixed) => compute_fixed_offset::<Axis>(scoped),

        CssValue::Keyword(CssKeyword::Sticky) => {
            compute_sticky_offset::<Axis>(scoped, normal_flow_offset)
        }
...
fn compute_relative_offset<Axis>(scoped: &mut ScopedDb, normal_offset: Subpixels) -> Subpixels
where
    Axis: crate::LayoutsMarker + 'static,
{
...
fn compute_absolute_offset<Axis>(scoped: &mut ScopedDb, static_position: Subpixels) -> Subpixels
where
    Axis: crate::LayoutsMarker + 'static,
{
    use rewrite_css::{EndMarker, PositionOffsetQuery, StartMarker};
    ...
    let (cb_offset, cb_size) = if let Some(cb_node) = containing_block {
        let cb_offset = scoped.node_query::<crate::OffsetQuery<Axis>>(cb_node);
        let cb_size = scoped.node_query::<SizeQuery<Axis, ConstrainedMarker>>(cb_node);
        (cb_offset, cb_size)
    } else {
        // No positioned ancestor, use viewport (0, 0) with full size
        (0, get_viewport_size::<Axis>(scoped))
    };

    // Get padding of containing block
    let cb_padding_start = if let Some(cb_node) = containing_block {
        get_node_padding_start::<Axis>(scoped, cb_node)
    } else {
        0
    };
    ...
    if end != 0 {
        ...
        let cb_padding_end = if let Some(cb_node) = containing_block {
            get_node_padding_end::<Axis>(scoped, cb_node)
        } else {
            0
        };
...
fn compute_fixed_offset<Axis>(scoped: &mut ScopedDb) -> Subpixels
where
    Axis: crate::LayoutsMarker + 'static,
{
    use rewrite_css::{EndMarker, PositionOffsetQuery, StartMarker};
    ...
    // Get viewport size
    let viewport_size = get_viewport_size::<Axis>(scoped);
...
fn compute_sticky_offset<Axis>(scoped: &mut ScopedDb, normal_offset: Subpixels) -> Subpixels
where
    Axis: crate::LayoutsMarker + 'static,
{
    ...
    // Determine axis
    let axis = if std::any::TypeId::of::<Axis>() == std::any::TypeId::of::<BlockMarker>() {
        crate::Layouts::Block
    } else {
        crate::Layouts::Inline
    };
...
fn get_viewport_size<Axis>(scoped: &mut ScopedDb) -> Subpixels
where
    Axis: crate::LayoutsMarker + 'static,
{
...
fn get_node_padding_start<Axis>(scoped: &mut ScopedDb, node: NodeId) -> Subpixels
where
    Axis: crate::LayoutsMarker + 'static,
{
    use rewrite_css::{PaddingQuery, StartMarker};
...
fn get_node_padding_end<Axis>(scoped: &mut ScopedDb, node: NodeId) -> Subpixels
where
    Axis: crate::LayoutsMarker + 'static,
{
