...
pub fn compute_sticky_offset(
    ...
    let threshold =
        if std::any::TypeId::of::<crate::Layouts>() == std::any::TypeId::of::<crate::Layouts>() {
            match axis {
                crate::Layouts::Block => {
    ...
    let container_offset = match axis {
        crate::Layouts::Block => scoped.node_query::<crate::OffsetQuery<BlockMarker>>(scroll_node),
        crate::Layouts::Inline => {
            scoped.node_query::<crate::OffsetQuery<InlineMarker>>(scroll_node)
        }
    };

    // Get current scroll position
    let scroll_offset = match axis {
        crate::Layouts::Block => scroll_state.block_scroll,
        crate::Layouts::Inline => scroll_state.inline_scroll,
    };
    ...
    let cb_end = if let Some(cb_node) = containing_block {
        let cb_offset = match axis {
            crate::Layouts::Block => scoped.node_query::<crate::OffsetQuery<BlockMarker>>(cb_node),
            crate::Layouts::Inline => {
                scoped.node_query::<crate::OffsetQuery<InlineMarker>>(cb_node)
            }
        };
        let cb_size = match axis {
            crate::Layouts::Block => {
                scoped.node_query::<SizeQuery<BlockMarker, ConstrainedMarker>>(cb_node)
            }
            crate::Layouts::Inline => {
    ...
    let element_size = match axis {
        crate::Layouts::Block => scoped.query::<SizeQuery<BlockMarker, ConstrainedMarker>>(),
        crate::Layouts::Inline => scoped.query::<SizeQuery<InlineMarker, ConstrainedMarker>>(),
    };
...
pub fn has_sticky_threshold(scoped: &mut ScopedDb, axis: crate::Layouts) -> bool {
    use rewrite_css::{PositionOffsetQuery, PositionQuery, StartMarker};
...
pub fn calculate_sticky_boundaries(
    ...
    let container_offset = if let Some(scroll_node) = scroll_container {
        match axis {
            crate::Layouts::Block => {
                scoped.node_query::<crate::OffsetQuery<BlockMarker>>(scroll_node)
            }
            crate::Layouts::Inline => {
                scoped.node_query::<crate::OffsetQuery<InlineMarker>>(scroll_node)
            }
