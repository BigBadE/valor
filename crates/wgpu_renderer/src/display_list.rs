...
pub enum DisplayItem {
    ...
    Text { x: f32, y: f32, text: String, color: [f32; 3], font_size: f32 },
    /// Push a rectangular clip onto the stack (placeholder; enforced as a scissor in RenderState for now).
    BeginClip { x: f32, y: f32, width: f32, height: f32 },
...
impl DisplayList {
    ...
    pub fn new() -> Self { Self { items: Vec::new(), generation: 0 } }
    ...
    pub fn diff(&self, other: &DisplayList) -> DisplayListDiff {
        if self == other { DisplayListDiff::NoChange } else { DisplayListDiff::ReplaceAll(other.items.clone()) }
    }
        ...
        for item in &self.items {
            match item {
                DisplayItem::Rect { x, y, width, height, color } => {
                    ...
                    rects.push(DrawRect { x: *x, y: *y, width: *width, height: *height, color: rgb })
                }
                DisplayItem::Text { x, y, text, color, font_size } => texts.push(DrawText { x: *x, y: *y, text: text.clone(), color: *color, font_size: *font_size }),
                DisplayItem::BeginClip { .. } | DisplayItem::EndClip | DisplayItem::Opacity { .. } => {
...
#[derive(Debug, Clone, PartialEq)]
pub struct Quad { pub x: f32, pub y: f32, pub width: f32, pub height: f32, pub color: [f32; 4] }

/// A batch of drawing work that shares the same scissor rectangle.
#[derive(Debug, Clone, PartialEq)]
pub struct Batch { pub scissor: Option<(u32, u32, u32, u32)>, pub quads: Vec<Quad> }
...
pub fn batch_display_list(list: &DisplayList, framebuffer_width: u32, framebuffer_height: u32) -> Vec<Batch> {
    ...
    let intersect = |a: (u32, u32, u32, u32), b: (u32, u32, u32, u32)| -> (u32, u32, u32, u32) {
        let (ax, ay, aw, ah) = a; let (bx, by, bw, bh) = b;
        let x0 = ax.max(bx);
        let y0 = ay.max(by);
        ...
        match item {
            DisplayItem::Rect { x, y, width, height, color } => {
                ...
                if *width > 0.0 && *height > 0.0 {
                    current_quads.push(Quad { x: *x, y: *y, width: *width, height: *height, color: rgba });
                }
            }
            DisplayItem::BeginClip { x, y, width, height } => {
                ...
                let effective = match current_scissor { Some(sc) => intersect(sc, new_sc), None => new_sc };
    ...
    if !current_quads.is_empty() {
        batches.push(Batch { scissor: current_scissor, quads: current_quads });
    }
    batches
}
...
mod tests {
    ...
    fn diff_basic() {
        ...
        a.push(DisplayItem::Rect { x: 0.0, y: 0.0, width: 10.0, height: 10.0, color: [1.0, 0.0, 0.0, 1.0] });
        let mut b = a.clone();
        assert_eq!(a.diff(&b), DisplayListDiff::NoChange);
        b.push(DisplayItem::Text { x: 5.0, y: 5.0, text: "hi".to_string(), color: [0.0, 0.0, 0.0], font_size: 12.0 });
        match a.diff(&b) {
    ...
    fn flatten_basic() {
        let list = DisplayList::from_items(vec![
            DisplayItem::Rect { x: 1.0, y: 2.0, width: 3.0, height: 4.0, color: [0.5, 0.5, 0.5, 1.0] },
            DisplayItem::Text { x: 10.0, y: 20.0, text: "abc".into(), color: [0.0, 0.0, 0.0], font_size: 14.0 },
        ]);
    ...
    fn batching_with_clips() {
        let list = DisplayList::from_items(vec![
            DisplayItem::Rect { x: 0.0, y: 0.0, width: 50.0, height: 10.0, color: [1.0, 0.0, 0.0, 1.0] },
            DisplayItem::BeginClip { x: 5.0, y: 5.0, width: 10.0, height: 10.0 },
            DisplayItem::Rect { x: 6.0, y: 6.0, width: 2.0, height: 2.0, color: [0.0, 1.0, 0.0, 1.0] },
            DisplayItem::EndClip,
        ]);
